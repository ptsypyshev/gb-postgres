...
-- Create index for login on users
CREATE INDEX users_login_idx ON users (login);
-- Next index gives no effect
-- CREATE INDEX task_statuses_task_status_name_idx ON task_statuses (task_status_name);

CREATE INDEX attachment_membership_task_id_idx ON attachment_membership (task_id) INCLUDE (attachment_id);

CREATE INDEX task_membership_parent_id_idx ON task_membership (parent_id) INCLUDE (child_id);

CREATE INDEX task_control_membership_controller_id_idx ON task_control_membership (controller_id) INCLUDE (task_id);
